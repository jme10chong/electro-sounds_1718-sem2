...
const s1 = function(p) {
  ...
  let b = 1;
    ...
    let mid = sound.fft_mid.getEnergy("mid"); // very dynamic value
    ...
    for(let x = 0; x < 30; x++) {
      for(let y = 0; y < 15; y++) {
        ...
        p.push();
        p.translate(-treble, -treble);
        p.point(p.width-x*60*240/bass, y*40);
        p.pop();
      }
    }
  }
  ...
  superShape = function(m, a, b, size, choose) {
    ...
    for(let angle = 0; angle < p.TWO_PI; angle+=increment) {
      ...
      switch(choose) {
        case 0: p.vertex(x,y); p.vertex(-x,y); p.vertex(-x,-y); p.vertex(x,-y); break; // anti-clockwise
      }
